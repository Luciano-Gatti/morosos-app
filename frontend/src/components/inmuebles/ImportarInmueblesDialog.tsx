  ...
  const procesar = async () => {
    ...
    try {
      ...
      const detail =
        ...
        Array.isArray((error.data as { details?: Array<{ message?: string }> }).details)
          ? (error.data as { details: Array<{ message?: string }> }).details
              .map((d) => d?.message)
      ...
      const message =
        error instanceof ApiError
          ? [error.message, detail].filter(Boolean).join(": ")
